...
use cayley_wasm::graph::iterator::limit::{Limit};
...
fn test_limit_iterator_basics() {
    ...
    let expect_sz = all_it.borrow_mut().stats(&ctx);
    let sz = u.borrow_mut().stats(&ctx);
    ...
    assert!(!uc.borrow_mut().contains(&ctx, &Ref::new_i64_node(4)));

    let uc = u.borrow().lookup();
    for v in vec![5,4,3] {
        assert!(uc.borrow_mut().contains(&ctx, &Ref::new_i64_node(v)));
    }
    assert!(!uc.borrow_mut().contains(&ctx, &Ref::new_i64_node(2)));
}